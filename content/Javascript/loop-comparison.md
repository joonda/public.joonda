...
selectedRows.forEach((row) => {
  if (!row.itemnm || row.itemnm.trim() === '') {
    return vAlertError('품목명을 입력해주세요.');
  } else if (!row.unit || row.unit.trim() === '') {
    return vAlertError('단위를 입력해주세요.');
  } else if (row.reqqty == null || row.reqqty <= 0) {
    return vAlertError('요청수량을 입력해주세요.');
  }
});
```
...
```
* 값(value)를 순회 (return, break, continue 등 루프 제어 가능)

```javascript title="for in"
...
```
- 객체의 키를 순회 (return, break, continue 등 루프 제어 가능)
- `for in`은 배열에도 사용 가능하지만, **순서가 보장되지 않고** 상속된 키도 순회할 수 있으므로 배열에는 **사용을 권장하지 않는다**.